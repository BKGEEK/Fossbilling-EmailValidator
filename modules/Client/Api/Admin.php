...
class Admin extends \Api_Abstract
{
    ...
    public function get($data)
    {
        ...
        return $service->toApiArray($service->get($data), true, $this->getIdentity());
    }
     ...
     * @return array
     */
    public function email_validation_config($data)
    {
        $config = $this->di['mod_config']('client');
        
        return [
            'email_domain_validation_enabled' => $config['email_domain_validation_enabled'] ?? true,
            'email_alias_validation_enabled' => $config['email_alias_validation_enabled'] ?? true,
            'email_allowed_domains' => $config['email_allowed_domains'] ?? ['outlook.com,gmail.com,qq.com,163.com,loc.cc'],
        ];
    }
     ...
     * @return bool
     */
    public function update_email_validation_config($data)
    {
        $required = [
            'email_domain_validation_enabled' => 'Email domain validation status is required',
            'email_alias_validation_enabled' => 'Email alias validation status is required',
        ];
        ...
        $config = [
            'email_domain_validation_enabled' => (bool) $data['email_domain_validation_enabled'],
            'email_alias_validation_enabled' => (bool) $data['email_alias_validation_enabled'],
            'email_allowed_domains' => $data['email_allowed_domains'] ?? [],
        ];

        return $systemService->updateParams(['mod_client' => $config]);
    }
